...
const AGENTS = [
  { label: "Claude", bin: "claude", args: "-p", icon: ClaudeLogo, prompt: `"Use fal.ai to generate a logo for my startup called 'Moonshot Labs' - modern, minimal, space themed."` },
  { label: "Codex", bin: "codex", args: "--full-auto", icon: CodexLogo, prompt: `"Use fal.ai to generate a logo for my startup called 'Moonshot Labs' - modern, minimal, space themed."` },
...
export function AgentTabs() {
  ...
  return (
    <div className="not-prose flex flex-col gap-3" style={{ maxWidth: 620 }}>
      <div className="rounded-md overflow-hidden text-left" style={{ border: "1px solid var(--vocs-border-color-primary)", boxShadow: "0 1px 3px light-dark(rgba(0,0,0,0.08), rgba(0,0,0,0.2))" }}>
        {/* Agent tabs */}
        <div className="flex" style={{ background: "light-dark(var(--vocs-background-color-surfaceMuted), oklch(0.22 0 0))", borderBottom: "1px solid var(--vocs-border-color-primary)" }}>
          {AGENTS.map((a, i) => {
            const Icon = a.icon;
            return (
              <button key={a.label} type="button" onClick={() => setActive(i)} className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium transition-colors cursor-pointer" style={{ color: i === active ? "var(--vocs-text-color-heading)" : "var(--vocs-text-color-muted)", background: i === active ? "var(--vocs-background-color-surface)" : "transparent", borderRight: i < AGENTS.length - 1 ? "1px solid var(--vocs-border-color-secondary)" : "none" }}>
                <Icon className="w-4.5 h-4.5" />{a.label}
              </button>
            );
          })}
        </div>
