...
`tempo()` creates a `TempoMethod` that bundles a payment network, its intents, and default parameters together. By default it registers a `charge` intent. Pass additional intents (like `StreamIntent`) via the `intents` parameter.
...
Register both intents on a single method by passing a `StreamIntent` via `intents`:

```python [server.py]
from mpay.methods.tempo import tempo, StreamIntent
...
Additional intents to register, merged with the default `charge` intent. Use this to add stream support.
